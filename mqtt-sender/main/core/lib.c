...
void config(struct device *z)
{
	...
	z->isEnable[12] = isEnable_Heart_BPM;
	z->isEnable[13] = isEnable_SPO2;
...
void initPeripherals(long *c)
{
	*c = 0; // Init counter
	...
	init_ssd1306();
	//ssd1306_text("4Oximeter|||4SpO2|||BPM");
}
...
void getData(struct device *z, long *c)
{
	...
	if (check_max30102())
	{
		for (i = 0; i < 2; i++)
		{
			strcpy(z->d[i + 12], get_max30102(i));
		}
	}
	else
	{
...
void generateJson(struct device *z)
{
	...
	{
		aux = 0;
		strcat(z->json, ",{\"sensor\":\"MAX30102\",\"data\":[");
		for (i = 0; i < 2; i++)
		{
			if (z->isEnable[i + 12])
			{
				if (aux != i)
					strcat(z->json, ",");
				strcat(z->json, "{\"");
				strcat(z->json, z->s_name[i + 12]);
				strcat(z->json, "\":\"");
				strcat(z->json, z->d[i + 12]);
				strcat(z->json, "\"}");
			}
			else
				aux++;
...
bool sendtoScreen(struct device *dev)
{
    char textstr[64];
    sprintf(textstr,"4 -OXIMETER- ||| %s SpO2||| %s bpm", dev->d[13], dev->d[12]);
